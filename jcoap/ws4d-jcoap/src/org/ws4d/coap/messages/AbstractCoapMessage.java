...
public abstract class AbstractCoapMessage implements CoapMessage {
	...
	protected static final int HEADER_LENGTH = 4;	//it indicates the Header length which is 4 bytes
	
	/* Header */
	protected int version;				//version is fixed to 1 and remaining values are reservd
	protected CoapPacketType packetType;			//Whether CON,NON,ACK,RST message
	protected int messageCodeValue;			//whether it may be request(GET,POST,PUT,DEL) or response message
	//protected int optionCount; 				//Number of options present in this message
	protected int messageId;				//Unique Transaction ID assigned by the source
    ...
    protected void serialize(byte[] bytes, int length, int offset){//it fills the header and payload
	...
	protected static long coapUint2Long(byte[] data){
		...
		/* convert to long */
		//converting the byte array to byte buffer(wrapping an array of data around a buffer)
