...
public class BasicCoapSocketHandler implements CoapSocketHandler {
    ...
    public BasicCoapSocketHandler(CoapChannelManager channelManager, int port) throws IOException {
        ...
        dgramChannel = DatagramChannel.open();	//creating a datagram channel and binding it to a port
       	dgramChannel.socket().bind(new InetSocketAddress(port)); //If port is specified as zero ,then a free port is choosen from the available ports
        this.localPort = dgramChannel.socket().getLocalPort();	//storing the port(useful if the port is zero)
        dgramChannel.configureBlocking(false);		//making the datagramChannel non blocking
       
        workerThread = new WorkerThread();		//starting the working thread
        workerThread.start();
    }
    ...
    protected class WorkerThread extends Thread {
		...
		public WorkerThread() {
			dgramBuffer = ByteBuffer.allocate(UDP_BUFFER_SIZE);//allocating the buffer with memory 
		    startTime = System.currentTimeMillis();
		    try {
				selector = Selector.open();	//creating a selector object
				dgramChannel.register(selector, SelectionKey.OP_READ);//registering the channel with the selector with the selection key specified as OP_READ
		...
		public void run() {
			...
			while (dgramChannel != null) {
				...
				try {
					...
					//It will wait until waitFor milliseconds for a channel to be getting ready
		...
		private void handleIncommingMessage(ByteBuffer buffer, InetSocketAddress addr) {
			CoapMessage msg;
			try {
				//buffer.position() will return the position from which we can read or write the data
				//buffer.array() will return the array of the bytebuffer
		...
		private void sendUdpMsg(CoapMessage msg) {
			...
			/* send message*/
			//creating a bytebuffer from a byte array
	...
	//checking whether the connection to the host and port already exist
